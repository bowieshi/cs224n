...
class CharCorruptionDataset(Dataset):
    ...
    # version 1:
        # doc = self.data[idx]
        # low_bound, upp_bound = 4, int(self.block_size*7/8)
        # truncate_len = int(random.random()*(upp_bound-low_bound+1)+4)
        # start_pos = int(random.random()*((self.block_size-truncate_len)-0+1)+0)
        # # print(truncate_len)
        # truncate_doc = doc[start_pos:start_pos+truncate_len]
        # masked_len = min(int(np.random.beta(1, 3)*truncate_len)+1, truncate_len-2)
        # start_pos = int(random.random()*(truncate_len-masked_len-2)+1)
        # prefix = truncate_doc[0:start_pos]
        # masked_content = truncate_doc[start_pos:start_pos+masked_len]
        # suffix = truncate_doc[start_pos+masked_len:]
        # # print(start_pos, masked_len)
    ...
    # version 3
        ...
        mask_len = np.random.binomial(n=trunc_len - 3, p=(trunc_len - 4) / (
                    4 * (trunc_len - 3))) + 1  # binomial to determine mask length

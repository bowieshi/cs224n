...
if args.function == 'pretrain':
    ...
    if args.reading_params_path is not None: # finetuning with pretrain
        ...
        train_conf = trainer.TrainerConfig(max_epochs=75,
                                      batch_size=256,
                                      learning_rate=6e-4,
                                      lr_decay=True,
                                      warmup_tokens=512 * 20,
        ...
        train_conf = trainer.TrainerConfig(max_epochs=10,
                                      batch_size=256,
                                      learning_rate=6e-4,
                                      lr_decay=True,
                                      warmup_tokens=512 * 20,
    ...
    # initialize a trainer instance and kick off training
    trainer = trainer.Trainer(gpt, finetune_dataset, None, train_conf)
    trainer.train()
    torch.save(gpt.state_dict(), args.writing_params_path)

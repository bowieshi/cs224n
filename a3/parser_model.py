...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        #     nn.init.xavier_uniform_(torch.empty(self.embed_size, self.n_classes)))
    ...
    def embedding_lookup(self, w):
        ...
        # print(w.size())
        # x = torch.tensor()
        # for i in range(w.resize(dim=0)):
        #     x = torch.concat(x, torch.flatten(torch.index_select(self.embeddings, 0, w[_])))
        # print(x)
    ...
    def forward(self, w):
        ...
        h = self.dropout(torch.nn.functional.relu(torch.matmul(embed, self.embed_to_hidden_weight) + self.embed_to_hidden_bias))
